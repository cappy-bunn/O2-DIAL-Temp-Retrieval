...
if mod(window_t,2) == 0
    ...
    o2on_avg_transpose = interp1(t_o2on_shift,o2on_avg',t_o2on);
...
if mod(window_t,2) == 0
    ...
    o2off_avg_transpose = interp1(t_o2off_shift,o2off_avg',t_o2off);
...
% Surface temperature and pressure are split off in new data.
tsk_ret_v = T_surf;
psatm_ret_v = P_surf;

tsk_ret = tsk_ret_v(1,pro_num);
tsk = tsk_ret;
psatm_ret = psatm_ret_v(1, pro_num);
psatm = psatm_ret;
...
tk_ret = tsk_ret - 6.5.*rkm;             %  Assume a lapse rate of -6.5 K/km
tk_ret = tk_ret';                        % to stay consistant with DIAL_Temperature_Performance_v14_ksr.m program
T0 = tk_ret;
...
%  iteration.
fitdata = fitlm(rm, tk_ret);
fittable = fitdata.Coefficients;
fitgamma1 = fittable{'x1', 'Estimate'};
fitintercept1 = fittable{'(Intercept)', 'Estimate'};

%  =======================  Pressure profile  =============================
patm_ret = psatm_ret*(fitintercept1./(T0)).^(.0341626./fitgamma1);
pkpa_ret = 101.325*patm_ret;
...
i_range = size(rm, 2);
N_o2_on_noise = o2on_avg(:, pro_num);
N_o2_off_noise = o2off_avg(:, pro_num);

N_o2_on_noise = N_o2_on_noise';         % to stay consistant with DIAL_Temperature_Performance_v14_ksr.m program
N_o2_off_noise = N_o2_off_noise';

for j = 1:1:i_range-1-oversample
    ln_o2(j) = log(N_o2_on_noise(j)*N_o2_off_noise(j+oversample)/N_o2_on_noise(j+oversample)/N_o2_off_noise(j));
end
...
%  spectral distributionusing the guessed at temperature profile
doppler_o2_ret_un = ((c_doppler_o2./tk_ret).^.5).*exp(-c_doppler_o2.*(wno2_center-wn_o2_scan_t).^2./tk_ret);          
...
bmo2_ret = 374280*pkpa_ret./tk_ret./l_o2_on.^4;
...
% =========================================================================
%  estimating where dg_dr term will cause an error in the retrieved alpha
dg_dr_middle = dg_dr_o2_ret(middle, :);
dg_dr_square = dg_dr_middle.^2;
[dg_dr_max, dg_dr_max_index] = max(dg_dr_square(1,26:140));     % starts search after 1 km
dg_dr_max_index = dg_dr_max_index + 25;
rkm_max = rkm(dg_dr_max_index);
maxslope = dg_dr_middle(1, dg_dr_max_index);
if dg_dr_max_index < 15
    dg_dr_max_index = 30;
    maxslope = 0;
end
if dg_dr_max_index > size(rkm, 1)-15
    dg_dr_max_index = size(rkm, 1)-30;
    maxslope = 0;
end
if maxslope > 0
    signslope = -1;
...
iii = 1;
for jjj = dg_dr_max_index-2*oversample-1:1:dg_dr_max_index+2*oversample+1;
    dg_dr_o2_ret(middle, jjj) = dg_dr_o2_ret(middle, jjj)*(1+signslope.*magsign.*0);
end

%  ========================================================================
%  My justification here is the difference between the actual slope and the
%  slope calcualted using a difference
%  ========================================================================


%  ---  calculating the normalized molecular absorption lineshape absorption f
a_o2_ret = s0_o2.*(t0_o2./tk_ret);
c_o2_ret = exp(h.*c./kb.*(1./t0_o2-1./tk_ret).*E_o2);
sT_o2_ret = a_o2_ret.*c_o2_ret./100;                 % the divide by 100 makes the units m/molecule

gammaL_o2_ret = (gl0_o2*100).*(patm_ret./p0_o2).*(t0_o2./tk_ret).^alpha_o2;        % times 100 makes the units 1/m
gammaD_o2_ret = (wno2_center/c).*(2.*kb.*log(2).*tk_ret./mo2).^.5;               % units are 1/m
